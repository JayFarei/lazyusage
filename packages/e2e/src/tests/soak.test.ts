...
import { assertNoCrash, assertLayoutIntact, assertStatusBarPresent } from "../helpers/assertions.js";
...
describe("Soak test - 10 min runtime stability", () => {
  test(
    "TUI runs stably for 10 minutes without crash or memory leak",
    async () => {
      ...
      for (let i = 1; i <= samples; i++) {
        ...
        console.log(
          `  [${elapsed}s] Layout OK, RSS=${rss ?? "?"}KB`,
        );
      }
      ...
      if (rssReadings.length >= 2) {
        ...
        console.log(
          `  Memory: initial=${firstRSS}KB, final=${lastRSS}KB, growth=${growthFactor.toFixed(2)}x`,
        );
...
describe("Soak test - fallback graceful degradation", () => {
  test(
    "TUI starts and runs when API data is unavailable",
    async () => {
      if (skipIfNoTmux()) return;
