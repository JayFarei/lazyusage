...
### Fixed
- Stale cached usage now produces an explicit warning instead of appearing to be current when live providers are temporarily unavailable.

## [0.2.3] - 2026-07-07

### Fixed
- Claude usage parsing now handles Anthropic's new `limits[]` API shape for the Fable weekly limit while preserving the existing `week_sonnet` JSON/storage key for compatibility.
- Claude `/usage` PTY parsing accepts the new `Current week (Fable)` label and keeps the legacy `Sonnet only` fallback.
- CLI and TUI output now label the model-specific weekly budget as Fable.
