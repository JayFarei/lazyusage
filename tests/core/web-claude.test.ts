...
type ClaudeWebProviderTestStatics = { _rateLimitedUntil: number };
...
function setClaudeWebRateLimit(value: number): void {
  (ClaudeWebProvider as unknown as ClaudeWebProviderTestStatics)._rateLimitedUntil = value;
}

beforeEach(() => {
  setClaudeWebRateLimit(0);
...
describe("ClaudeWebProvider - isAvailable", () => {
  ...
  test("returns false when rate-limited", () => {
    setClaudeWebRateLimit(Date.now() + 60_000);
    const provider = new ClaudeWebProvider(makeMockCookies());
    expect(provider.isAvailable()).toBe(false);
  });
